...
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "x$bitcoin_qt_want_version" = "xauto" && test x$bitcoin_qt_force != xyes; then
    if test x$bitcoin_enable_qt != xno; then
      AC_MSG_WARN([$1; paccoin-qt frontend will not be built])
    fi
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build paccoin-qt GUI (default=auto, qt5 tried first)])],
    [
...
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  dnl enable qt support
  AC_MSG_CHECKING(whether to build Paccoin Core GUI)
  BITCOIN_QT_CHECK([
...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS_WITH_PKGCONFIG],[
  ...
  if test x$auto_priority_version = x; then
    ...
    qt5_modules="Qt5Core Qt5Gui Qt5Network Qt5Widgets Qt5WebChannel Qt5WebSockets"
    qt4_modules="QtCore QtGui QtNetwork QtWebChannel QtWebSockets"
    BITCOIN_QT_CHECK([
      if test x$bitcoin_qt_want_version = xqt5 || ( test x$bitcoin_qt_want_version = xauto && test x$auto_priority_version = xqt5 ); then
